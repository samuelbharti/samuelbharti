### Hi, I’m Sam 👋

I’m a product-minded computational biologist. I like building tools that make omics data easier to explore, trust, and share.
I build interactive apps and AI-assisted tools for genomics and multi-omics integration, spanning bulk, single-cell, and Perturb-seq RNA-seq through whole-exome data, with a focus on modular design and reproducible workflows.

[![Portfolio](assets/badges/portfolio.svg)](https://samuelbharti.com)
[![LinkedIn](assets/badges/linkedin.svg)](https://www.linkedin.com/in/samuelbharti/)
[![ORCID](assets/badges/orcid.svg)](https://orcid.org/0000-0003-4190-7058)
[![Email](assets/badges/email.svg)](mailto:samuelbharti.io@gmail.com)

#### Now
- **Software Engineering Intern, Shiny team @ [Posit](https://posit.co/)** (Summer 2026), building Shiny apps for life sciences.
- **Doctoral Researcher in Bioinformatics @ UAB.**
- Previously: Human Genetics (gRED) intern @ Genentech (Summer 2025).
- Building agentic AI tools for scientific data exploration.
- 8 peer-reviewed publications in computational biology & genomics ([ORCID](https://orcid.org/0000-0003-4190-7058)).

#### Toolbox

- **Languages** R · Python · Bash · SQL · JavaScript
- **Apps & viz** Shiny · Quarto · React · Node.js · Leaflet · dygraphs
- **Genomics** Seurat · nf-core · bulk/sc/Perturb-seq · WES · ATAC-seq · eQTL/GWAS
- **Agentic AI** LangChain · MCP · Google ADK · OpenAI APIs · tool calling
- **Infra** Docker · AWS · GCP · SLURM/HPC · Git

#### Featured projects

**Agentic AI for science**
- **genescout**: turn a candidate gene list + disease context into a ranked, evidence-supported review. Deterministic keyless core, optional Claude agent layer.
  Repo: https://github.com/samuelbharti/genescout · Hackathon: https://www.samuelbharti.com/claude-science-hackathon.html
- **biobouncer**: a gate for biological inputs that validates gene symbols, ontology terms, variant formats, and database identifiers.
  Repo: https://github.com/samuelbharti/biobouncer · Docs: https://www.samuelbharti.com/biobouncer/
- **RAPTOR**: agentic system that extracts phenotypes, genes, diseases, and ontology-linked concepts from unstructured patient records (unreleased).

**R / Shiny tooling**
- **R Shiny Template**: reusable template for bioinformatics web apps.
  Repo: https://github.com/samuelbharti/RShiny_template
- **Peacock**: R package for project initialization/workflow management.
  Repo: https://github.com/samuelbharti/peacock · Docs: https://www.samuelbharti.com/peacock/
- **LLM in R-Shiny**: drop-in chatbot module for Shiny apps.
  Repo: https://github.com/samuelbharti/RShiny_LLM_Module
- **Install my R packages** via r-universe: https://samuelbharti.r-universe.dev/

**Genomics apps & analysis**
- **MOLV (Multi-Omics Locus Viewer)**: locus-first Shiny app + R package built at Genentech to explore 11,000+ GWAS, eQTL, pQTL, single-cell, and ATAC-seq datasets in Alzheimer's disease (private)
- **scRNA-seq Analysis Integration App (in dev)**: nf-core outputs + Seurat + pseudobulk + CellChat in one place (unreleased)
- **Pediatric Thyroid Cancer Explorer**: interactive WES/bulk RNA exploration.
  Repo: https://github.com/uab-cgds-worthey/ptc-explorer
- **SEAS**: clinical feature enrichment + prediction.
  App: https://aimed-lab.shinyapps.io/SEAS/ · Docs: https://aimed-lab.github.io/SEAS/

**Databases & earlier work**
- **PAGER / PAGER Web App**: pathway + gene-set enrichment tooling + network views.
  Site: http://discovery.informatics.uab.edu/PAGER/ · Repo: https://github.com/aimed-lab/PAGER-Web-APP
- **sMAP**: transcriptomics QC + stats + biomarker discovery (built for non-coders).
  Docs: https://bi-stem-away.github.io/sMAP/ · Demo PDF: https://mentorchains.github.io/STEM-AWAY_AWS_BI_2021/sMAP_demo.pdf · Repo: https://github.com/BI-STEM-Away/sMAP
- **BioDivPortal (Poland)**: biodiversity exploration app.
  Repo: https://github.com/samuelbharti/BioDivPortalPoland · App: https://samuelbharti.shinyapps.io/BioDivPortal/
- **PepEngine**: structural database for synthetic peptides. https://pepengine.in/
- **GlucoKinaseDB**: curated database of glucokinase modulators. https://glucokinasedb.in/ · Dataset/API: https://glucokinasedb.in/dataset
- **VIRdb 2.0**: vitiligo resource

#### Teaching & outreach
- Instructor at Carpentry Workshop on "R and Git" at UAB. https://u-bds.github.io/2026-01-29-uab/
- Instructor at Carpentry Workshop on "Bash Shell, Git, Text Editor, and Python" at UAB. https://u-bds.github.io/2024-03-18-uab/
- Instructor at Carpentry Workshop on "Bash Shell, Git, Text Editor, and R" at UAB. https://u-bds.github.io/2023-12-14-uab/
- Helper at Carpentry Style Workshop on "Bulk RNA-Seq" at UAB. https://u-bds.github.io/2024-07-09-uab/
- Flash Talk on R Shiny Bioinformatics app on AWS EC2. https://mentorchains.github.io/STEM-AWAY_AWS_BI_2021/
- **Additional Resource**: R For Beginners (WEB + Quarto Slides) https://github.com/samuelbharti/r-for-beginners

#### Elsewhere
[Portfolio](https://samuelbharti.com) · [GitHub](https://github.com/samuelbharti) · [LinkedIn](https://www.linkedin.com/in/samuelbharti/) · [ORCID](https://orcid.org/0000-0003-4190-7058) · [Email](mailto:samuelbharti.io@gmail.com)
